...
A Firefox extension that boosts audio volume up to **600%** on any tab. Clean popup UI, per-site memory, EQ filters, and zero data collection.
...
Volume changes are sent from the popup to the content script via `browser.tabs.sendMessage`. State is stored locally with `browser.storage.local`, keyed by domain.
...
If you switch filter modes or add the current site to a block/allow list while audio is boosted, Gain now resets that tab back to a neutral 100% state automatically.
